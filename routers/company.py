...
def read_company(db: Session = Depends(get_db), user: User = Depends(get_current_user)):
...
def read_all_companies(db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    if user.is_staff:
