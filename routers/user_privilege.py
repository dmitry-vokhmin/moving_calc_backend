...
def get_all_user_privileges(db: Session = Depends(get_db), user_id=Depends(get_current_user)):
...
@router.get("/user_privilege/",
            response_model=List[user_privilege_schema.UserPrivilegeGet],
            status_code=status.HTTP_200_OK)
def get_all_user_privileges(db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    if user.is_staff:
...
@router.get("/user_privilege/{user_privilege_id}",
            ...
            status_code=status.HTTP_200_OK)
def get_user_privilege(user_privilege_id: int, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    if user.is_staff:
