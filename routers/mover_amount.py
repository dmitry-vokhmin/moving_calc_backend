...
@router.put("/mover_amount/delete/{mover_amount_id}", status_code=status.HTTP_200_OK)
def delete_mover_amount(mover_amount_id: int, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    if user.is_staff:
...
@router.put("/mover_amount/update/{mover_amount_id}", status_code=status.HTTP_200_OK)
